...
# CLASSES
class Sensor:
    def __init__(self, std_distance=0.01, std_angle=0.001, max_range=5.0):
    ...
    def update_error(self, measurements, treshold=2.0):
        ...
        for measurement in measurements:
            ...
            if distance < treshold:
                # assumption: projected landmark and its closest map landmark are identical
                angle_expected, distance_expected = measurement_model(self.get_pose(), self.map[best_index])
                error_distance = abs(distance_measured - distance_expected)**2
                error_angle = abs(angle_measured - angle_expected)**2
                self.error += error_distance + error_angle

            else:
                # assumption: projected landmark is a new landmark -> store to map
                self.map.append((x_projected, y_projected))
                self.error += 0.2 + 0.1 * min_distance
...
class Slam:
    ...
    def update(self, angle, step, measurements):
        for p in self.particles:
            p.move(angle, step)
            p.update_error(measurements)
    
    def resample(self):
        weights = [math.exp(-p.error * 20) for p in self.particles]
...
if __name__ == "__main__":
    ...
    robot = Robot()
    slam = Slam(num_particles=400, treshold=3.0)
    sensor = Sensor(max_range=5.0)
